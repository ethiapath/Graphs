...
if len(roomExits) == 1:
    # go back
    player.travel(roomExits[0])

the main interface is player object

so the basic idea is to dft until you reach a deadend 
then backtrack with bft until you find another unvisted path room

on every move add to visted_rooms
'''
...
def map_rooms_around_player():
    ...
    for d in player.currentRoom.getExits():
        if d not in maze_map[player.currentRoom.id]:
            maze_map[player.currentRoom.id][d] = '?'
        q.append(d)
    while len(q) > 0:
        ...
        print(prevRoom, direction)
        maze_map[prevRoom][direction] = currentRoom
        goBack(direction)
    
'''
print(maze_map)

# graph for small room
small_room_graph = {0: {'n': 1, 's': 5, 'w': 7, 'e': 3}, 5: {'n': 0, 's': 6}, 6: {'n': 5, 'w': 11}, 11: {'w': 10, 'e': 6}, 10: {'n': 9, 'e': 11}, 9: {'n': 8, 's': 10}, 8: {'s': 9, 'e': 7}, 7: {'w': 8, 'e': 0}, 3: {'w': 0, 'e': 4}, 4: {'w': 3}, 1: {'n': 2, 's': 0}, 2: {'s': 1}}

# now how do we turn this graph into cardinal directions...

I guess finding the entire graph isn't that useful since you still need to 
transform that into a list of directions the player can use to move
'''
...
def goBack(direction):
    if debug:
        clear()
        world.printRooms(player.currentRoom)
        print(f'go back from {direction} to {back_map[direction]}')
        time.sleep(DELAY)
    traversalPath.append(back_map[direction])
    player.travel(back_map[direction])

def fillOutMap(room):
    if room.id not in maze_map:
        maze_map[room.id] = {}
        roomExits = room.getExits()
        for direction in roomExits:
            maze_map[room.id][direction] = '?'

# bfs on maze_map to find closest unexplored node
# with path reverse lookup to find directions

# adding to traversalPath whenever the player moves gives a correct solution 
# but might be more inefficent then something else
...
def explore(direction, prevRoomId):
    ...
    for move in currentRoom.getExits():
        # i think the map I'm building is completly broken
        # as long as it replaces the '?' the algo works
        if maze_map[currentRoom.id].__contains__(move) and maze_map[currentRoom.id][move] == '?':
            explore(move, currentRoom.id)
    goBack(direction)
...
# for d in player.currentRoom.getExits():
#     s.push
s.append(player.currentRoom.getExits()[0])
while len(s) > 0:
    # load unexplored directions into stack?
    fillOutMap(player.currentRoom)
    roomId = player.currentRoom.id
    for direction in maze_map[roomId]:
        if maze_map[roomId][direction] == '?':
            # go that way
            # put direction taken into stack and append to traversalPath
            directionTaken = s.pop()
            traversalPath.append(directionTaken)
            # move player
            # if at deadend move by poping from stack until a new direction
'''
